...
"$tool/bin/devcontainer-image" explain security --report "$reports_dir" \
  | jq -e '(.dockerDaemonBakedIntoImage | not) and (.dockerSocketMountedByDefault | not) and .lifecycleLogRedaction and .extensionArtifactsLocked and (.uvxAutoRunFromShellInit | not) and (.npxAutoRunFromShellInit | not)' >/dev/null
