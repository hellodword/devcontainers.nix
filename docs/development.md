...
```sh
tests/smoke/run-plan.sh nix
```

Validate remote Docker CLI behavior with a reachable TCP daemon:

```sh
DOCKER_HOST=tcp://172.17.0.1:2375 tests/smoke/run-plan.sh nix
```

For release smoke where Docker daemon access is mandatory:

```sh
SMOKE_REQUIRE_DOCKER_DAEMON=1 DOCKER_HOST=tcp://172.17.0.1:2375 tests/smoke/run-plan.sh nix
```
